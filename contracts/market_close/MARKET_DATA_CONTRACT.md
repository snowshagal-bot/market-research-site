...
- KOSPI·KOSDAQ 거래대금이 당일 `확정`
- 수급 집중도(`market_internals.concentration`)는 final 필수 항목이 아니다. KRX 종목별 투자자 표는 16시 이후 공개되고 애프터마켓 누적이 계속 반영되어 정규장 값이 없으므로, 정규장 확정분이 없으면 빈 객체 `{}`로 둔다(16:05 이후 값을 정규장 값으로 넣지 않는다).
