...
- `1.2.0` includes `section_status`; a `partial`/`unavailable` last-5-session window carries `markets: {}` and `used_trading_days: 0`, unavailable KRX groups are `null`, and every indicator listed in `section_status.global_indicators.unavailable` carries no value (freshness rules skip only those listed indicators; KOSPI and KOSDAQ are never exempt);
