...
Updated: 2026-08-30
Branch: `feat/market-krx-groups-contract`
Contract: current `1.1.0`, legacy `1.0.1` supported
...
- Checkpoint 1 UI is complete:
  - initially copied the v1.0.1 contract, JSON Schema, and example payload; the current feature branch extends the compatibility contract to v1.1.0;
...
- Checkpoint 2 implementation is complete:
  ...
  - added `POST /api/market/publish` with a 512KB limit, full schema validation, final-only gates, and separate automated/admin header authentication; the current branch also permits authenticated writes on isolated branch Preview hosts;
...
- Final Preview route checks:
  ...
  - historical baseline: `POST /api/market/publish` returned HTTP 403 on Preview before the isolated Preview-write path was added;
  - historical baseline: `GET /api/market/latest` returned HTTP 503 `DB_NOT_CONFIGURED` when no Preview `COMMENTS_DB` binding was configured, followed by the explicitly labeled client-side Preview fixture.
...
- After explicit merge approval in a separate step, confirm the first Production `GET /api/market/latest` initializes `market_close_snapshots`, then publish the first final v1.1.0 snapshot while retaining v1.0.1 read compatibility.

## 현재 동작 상태

- Production and `main` are unchanged. The KRX groups extension exists only on the feature branch until review and merge.
...
- Review the KRX groups feature PR and its isolated Preview D1 evidence. Do not merge or configure/write Production from this task.

## 주의사항

- `schema_version`, field names, types, units, and meanings are fixed by the v1.0.1/v1.1.0 compatibility contract and must not be reinterpreted by the website.
