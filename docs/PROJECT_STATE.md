...
- HOME: USD/KRW, US 10Y and GOLD may be overlaid. When any card is, every card carries a small basis line — KRX `9/23 · 종가`, Global Latest `9/25 · 장중` / `9/25 · 종가`, a snapshot figure kept `9/23 · 마감 시점` (captured with the KRX close) or `9/22 · 종가` (a completed close) — because the strip then shows more than one date; with no overlay the strip is exactly the pre-B3 strip. The heading date stays the KRX close.
- MARKET TODAY: Market Close and Global Latest are requested together (Global Latest at most once per TODAY load, never on a direct HISTORY/1W/1M entry, never waited for more than 2.5 s). An overlaid card shows the observation's value, change, previous close and `source_date · 장중/종가` (intraday adds `as_of` in KST); the snapshot's 15:30 fixing and open/high/low are omitted there. KRX cards and every KRX section are the snapshot, unchanged. `render()` applies the overlay only when passed by the TODAY load and only while the mode is TODAY; each load takes a token so a slow TODAY response can never paint over HISTORY.
- Failure: a Global Latest timeout, error, malformed body or missing item leaves exactly the Market Close view.
- Tests: `tests/global-latest-overlay.test.mjs` (policy, MARKET TODAY/HISTORY/1W/1M, KO/EN), the B3 cases in `tests/home-initial-ssr.test.mjs` (SSR/hydration parity, zero first-load requests), fixtures in `tests/helpers/global-latest-fixtures.mjs`.
...
takeaway D1 → same-date same-locale Daily → hidden; USD/KRW, US 10Y and GOLD overlaid from the
`/api/market/global/latest` handler when fresh — see "Global Latest overlay (B3)"). Market, notice
and Global Latest are read in parallel within 1.5 s and fail independently; a failed or slow source is left to `assets/site.js`, which fetches it
...
- a 450px WebP thumbnail beside every cover (`covers/<id>-450.webp`), recorded in `posts.json` as `coverThumbnail` only when the file was actually committed. The homepage latest cards and the hero featured cover render from that field alone: with it they offer `srcset="…-450.webp 450w, <original> 900w"` with layout-measured `sizes` (`(max-width: 760px) 30vw, 112px` for cards, `(max-width: 760px) 140px, 220px` for the hero), so a 112px card downloads the small file and only a 3× desktop hero takes the original; without it they are the plain original `<img>`, never a guessed `-450` path (a missing srcset candidate is a broken image, not a fallback to `src`). The report page, share cards and social images never reference a thumbnail. Category landings' featured cards (`categoryFeaturedCards` on the server, `assets/category-landing.js` in the browser) use the same field and the same two files with their own `sizes`, `CATEGORY_FEATURED_COVER_SIZES`: the card paints the cover with `object-fit: contain` inside a 16:10 box (16:9 up to 680px), so the painted width — 123–149px on a phone, 146px at 768, 235px once the grid caps at 1180px — is what `sizes` states, not the box width. The first featured card's cover is the landing's largest contentful paint, so it alone is `loading="eager"` (`coverImageMarkup(post, sizes, { loading: 'eager' })`; every other cover, including the second card and the homepage cards, keeps the lazy default) — lazy, the browser only requested it after all seven stylesheets had arrived and laid out. The box, the contain fit and the card geometry are unchanged. `node scripts/build-cover-thumbnails.mjs` regenerates missing or stale thumbnails with the same Chrome-canvas WebP 0.9 encoding the covers use and brings the `coverThumbnail` records into line with the files on disk, rebuilding `posts.js` and the search index when they change.
...
The repository root also contains a non-indexable `404.html`. Its presence disables Cloudflare Pages' homepage SPA fallback for unknown paths, so missing public and report URLs return HTTP 404. The shared middleware preserves redirects and error statuses without injecting the report shell or report SEO metadata. The canonical public report URL is extensionless (`/reports/<path>`); canonical, hreflang, sitemap, JSON-LD and every internal link use it, and an existing report's `/reports/<path>.html` answers one `301` to it with the query string preserved (`legacyReportHtmlRedirect` in `functions/_middleware.js`). `data/posts.json` keeps the raw `.html` file path for publishing and file operations. A report page's final `<html lang>` is its matched post's `lang` (the URL decides only when no post matches), set by the middleware so an uploaded file written with the wrong `lang` is served correctly without editing it.
