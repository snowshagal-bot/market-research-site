...
12. **Research body pictures become files** (`functions/_body-assets.js`, used by `/api/publish` and by `/api/manage` on HTML replacement, only when the canonical post type is `research`). A Research upload carries its illustrations as base64 data URIs — 0.5–1MB inside a 1.1MB document — which held the parser, the report shell and DOMContentLoaded until the whole document had streamed and transferred every below-fold picture regardless of scrolling. The helper rewrites only `<img>` tags whose `src` is a base64 WebP/PNG/JPEG data URI: the decoded bytes must carry that format's magic and yield a width and height; the cover (an `<img>` inside a block element carrying one of `COVER_CONTAINERS`) and the first raster image outside such a container ("uncertain") stay inline, as do CSS `url(data:…)`, SVG, `<picture>`/`srcset` images and anything malformed. Files are content-addressed, `report-assets/<post-id>/<sha256 16 hex>.<webp|png|jpg>` — same bytes, same URL; changed bytes, new URL, no cache to invalidate — and `width`/`height` are added only when both are absent. `loading` and every other attribute are preserved verbatim; nothing is re-encoded. Guards apply before any decoding: more than 15 candidates or a candidate that could exceed 1MB passes the whole document through untouched, and the API response and server log report `converted` / `skipped` / reasons. Every file becomes a Git blob before the tree is built, so the HTML that names a file is only ever committed with it; the post records `bodyAssets` (HTML order, once each) for exactly the files in that commit. On replacement the files the new HTML no longer uses are deleted in the same commit, never before it; deletion removes them all. Daily, Weekly, Note and Basics uploads are byte-for-byte untouched. Existing Research reports are not migrated.
...
- refuses to delete report or cover paths outside the managed `reports/` and `covers/` directories, and body-picture paths that are not the post's own `report-assets/<post-id>/<hash>.<ext>` files;
- replacing a Research report's HTML runs the same body-picture extraction as publishing (see the publishing flow), committing new files, the rewritten HTML, `bodyAssets` and the deletion of files the new HTML no longer uses in one commit; deleting a post removes all of its recorded body files in the same commit.
...
- `functions/_body-assets.js` — Research body pictures out of the HTML into content-addressed `report-assets/` files; the plan both publish and manage commit atomically, and the invariants `tests/report-body-assets-invariants.test.mjs` enforces
