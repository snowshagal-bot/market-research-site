...
6. 현재 공개 메뉴 명칭은 `데일리 / 위클리 / 비정기 / 끄적끄적`이다. 내부 타입은 `daily / weekly / research / note`를 유지한다.
...
`docs/ROADMAP.md`의 "Next action"부터 시작한다. 현재 댓글용 D1 binding `COMMENTS_DB`는 Cloudflare에 저장된 상태이며, 다음 배포 후 실제 댓글 작성/삭제 및 모바일·데스크탑 검수가 남아 있다.
